...
1. Just like with System-Calls `Context Switch` also requires switching between `user mode` to `kernel mode`. Every native Thread is associated with its own `user stack` and `kernel stack` respectively: 
