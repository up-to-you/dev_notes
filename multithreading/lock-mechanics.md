...
2. `unpark()` time is not deterministic, due to OS Thread Scheduler mechanics. After `unpark()` System-call was invoked, it may take a long time to real Thread awakening, as there could be other busy Threads in OS, which Scheduler can decide to execute instead of those currently requested for awakening.
